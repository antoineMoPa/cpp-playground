...
int main(int argc, char ** argv){
    CImg<unsigned char> ones(100,100,1,3,255);
    
    for(char letter = 'a'; letter <= 'z'; letter++){
        CImg<int> letter_tot(100,100,1,3,0);
        CImg<unsigned char> letter_avg(100,100,1,3,0);
        int image_count = 0;
        ...
        for(auto& p: fs::directory_iterator("set/fonts")){
            ...
            if(fs::exists(path)){
                ...
                current_image.resize(100,100,1,3,0, 1, 0.5, 0.5);
                current_image = ones - current_image;
                
                letter_tot += current_image;
                image_count++;
                
                //CImgDisplay display(letter_tot.resize(100, 100));
                //pause();
                //break;
            }
        }
